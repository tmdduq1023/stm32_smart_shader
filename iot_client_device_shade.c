...
//                      printf("num_fields : %d \n",num_fields);
                ...
                else if (!strcmp(pArray[1], "SEND") && i == 6)
                {
                        ...
                        temp = atof(pArray[2]);
                        humi = atof(pArray[3]);
                        int var = atoi(pArray[4]);  // 수분
                        int cds = atoi(pArray[5]);  // 조도
                        printf("2\n");
    // INSERT INTO shade 테이블에 저장
                        sprintf(sql_cmd,"insert into shade(name, date, time, temp, humi, var, cds) values('%s', NOW(), NOW(), %.lf, %.lf, %d, %d)",pArray[0], temp, humi, var, cds);
                        printf("3\n");
                        res = mysql_query(conn, sql_cmd);
                        if (!res)
                        {
